...
Lunarglow - Open source implementation of NVIDIA's GameStream

=head1 SYNOPSIS

Usage: I<lunarglow> E<lt>actionE<gt> [options] [host] [-port E<lt>numberE<gt>]
       I<lunarglow> E<lt>configfileE<gt>
...
By default the encryption keys are stored in $XDG_CACHE_DIR/lunarglow or ~/.cache/lunarglow
...
By default the gamecontrollerdb.txt provided by Lunarglow is used.

=item B<-platform> [I<PLATFORM>]

Select platform for audio and video output and input.
<PLATFORM> can be x11, x11_vdpau, x11_vaapi, sdl or fake.
...
The default value is 'sysdefault' for ALSA.
...
Lunarglow will load a configuration file from:

  $XDG_CONFIG_HOME/lunarglow/lunarglow.conf (fallback to ~/.config/lunarglow/lunarglow.conf)

If no user specified configuration file is available the configuration will be loaded from:

  /etc/lunarglow/lunarglow.conf

A documented example configuration file can be found at /etc/lunarglow/lunarglow.conf.
